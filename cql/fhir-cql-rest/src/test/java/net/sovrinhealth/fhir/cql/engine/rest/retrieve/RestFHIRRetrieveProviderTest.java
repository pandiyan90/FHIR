 ...
 */
package net.sovrinhealth.fhir.cql.engine.rest.retrieve;
...
import net.sovrinhealth.fhir.cql.engine.util.FHIRClientUtil;
...
public class RestFHIRRetrieveProviderTest extends R4RestFHIRTest {
    ...
    public void multiplePagesAllReadSuccess() throws Exception {
        Condition c = Condition.builder().id("c1").subject(Reference.builder().reference(net.sovrinhealth.fhir.model.type.String.of("Patient/123")).build()).build();

        Bundle page1 =
                Bundle.builder().type(BundleType.SEARCHSET).total(UnsignedInt.of(1)).entry(Bundle.Entry.builder().resource(c).build()).link(Link.builder().relation(net.sovrinhealth.fhir.model.type.String.of("next")).url(Uri.of(getBaseUrl()
