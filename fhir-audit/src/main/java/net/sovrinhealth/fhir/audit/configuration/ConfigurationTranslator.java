 ...
 */
package net.sovrinhealth.fhir.audit.configuration;

import static net.sovrinhealth.fhir.audit.AuditLogServiceConstants.DEFAULT_AUDIT_KAFKA_TOPIC;
import static net.sovrinhealth.fhir.audit.AuditLogServiceConstants.DEFAULT_MAPPER;
import static net.sovrinhealth.fhir.audit.AuditLogServiceConstants.FIELD_LOAD;
import static net.sovrinhealth.fhir.audit.AuditLogServiceConstants.KAFKA_USERNAME;
import static net.sovrinhealth.fhir.audit.AuditLogServiceConstants.PROPERTY_AUDIT_KAFKA_APIKEY;
import static net.sovrinhealth.fhir.audit.AuditLogServiceConstants.PROPERTY_AUDIT_KAFKA_BOOTSTRAPSERVERS;
import static net.sovrinhealth.fhir.audit.AuditLogServiceConstants.PROPERTY_AUDIT_KAFKA_TOPIC;
import static net.sovrinhealth.fhir.audit.AuditLogServiceConstants.PROPERTY_AUDIT_MAPPER;
import static net.sovrinhealth.fhir.audit.configuration.type.KafkaType.KAFKA_BOOTSTRAP_SERVERS;
import static net.sovrinhealth.fhir.audit.configuration.type.KafkaType.KAFKA_DEFAULT_SASL_MECHANISM;
import static net.sovrinhealth.fhir.audit.configuration.type.KafkaType.KAFKA_DEFAULT_SECURITY_PROTOCOL;
import static net.sovrinhealth.fhir.audit.configuration.type.KafkaType.KAFKA_DEFAULT_SERIALIZER;
import static net.sovrinhealth.fhir.audit.configuration.type.KafkaType.KAFKA_DEFAULT_SSL_ENDPOINT_IDENTIFICATION_ALGORITHM;
import static net.sovrinhealth.fhir.audit.configuration.type.KafkaType.KAFKA_DEFAULT_SSL_PROTOCOL;
import static net.sovrinhealth.fhir.audit.configuration.type.KafkaType.KAFKA_KEY_SERIALIZER;
import static net.sovrinhealth.fhir.audit.configuration.type.KafkaType.KAFKA_SASL_JAAS_CONFIG;
import static net.sovrinhealth.fhir.audit.configuration.type.KafkaType.KAFKA_SASL_MECHANISM;
import static net.sovrinhealth.fhir.audit.configuration.type.KafkaType.KAFKA_SECURITY_PROTOCOL;
import static net.sovrinhealth.fhir.audit.configuration.type.KafkaType.KAFKA_SSL_ENABLED_PROTOCOLS;
import static net.sovrinhealth.fhir.audit.configuration.type.KafkaType.KAFKA_SSL_ENDPOINT_IDENTIFICATION_ALGORITHM;
import static net.sovrinhealth.fhir.audit.configuration.type.KafkaType.KAFKA_SSL_PROTOCOLS;
import static net.sovrinhealth.fhir.audit.configuration.type.KafkaType.KAFKA_VALUE_SERIALIZER;
...
import net.sovrinhealth.fhir.audit.configuration.type.IBMEventStreamsType;
import net.sovrinhealth.fhir.audit.configuration.type.KafkaType;
import net.sovrinhealth.fhir.audit.mapper.MapperType;
...
public class ConfigurationTranslator {
    ...
    private static Map<String, String> generateMap() {
        ...
        mapped.put("net.sovrinhealth.fhir.audit.logging.impl.DisabledAuditLogService", "net.sovrinhealth.fhir.audit.impl.NopService");
        mapped.put("net.sovrinhealth.fhir.audit.logging.impl.WhcAuditCadfLogService", "net.sovrinhealth.fhir.audit.impl.KafkaService");
        return mapped;
    }
    ...
    public void environment(Properties props, PropertyGroup auditLogProperties) throws Exception {
        ...
        if (System.getenv(IBMEventStreamsType.KUB_BINDING) != null) {
            ...
            net.sovrinhealth.fhir.audit.configuration.type.IBMEventStreamsType.EventStreamsCredentials credentials = IBMEventStreamsType.getEventStreamsCredentials();
            if (credentials != null) {
