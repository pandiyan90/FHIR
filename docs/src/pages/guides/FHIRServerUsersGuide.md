...
To enable tracing, set the TRACE_SPEC variable (e.g. by setting an environment variable by the same name) to a `:`-delimited list of `component = level` pair values. For example, to turn on tracing for the SQL generated by `fhir-persistence-jdbc`, set the TRACE_SPEC to `net.sovrinhealth.fhir.persistence.jdbc.dao.impl.*=fine:net.sovrinhealth.fhir.database.utils.query.QueryUtil=fine`.
...
``` json
    {
        "fhirServer": {
            …
            "persistence": {
                "factoryClassname": "net.sovrinhealth.fhir.persistence.jdbc.FHIRPersistenceJDBCFactory",
                …
            }
    }
```
...
1. Implement each operation as a Java class that extends `net.sovrinhealth.fhir.operation.AbstractOperation` from `fhir-operation.jar`. Ensure that your implementation returns an appropriate `OperationDefinition` in its `getDefinition()` method, because the framework validates both the request and response payloads to ensure that they conform to the definition.

2. Create a file named `net.sovrinhealth.fhir.operation.FHIROperation` with one or more fully qualified `FHIROperation` classnames and package it in your jar under `META-INF/services/`.
...
A persistence interceptor implementation must implement the `net.sovrinhealth.fhir.server.spi.interceptor.FHIRPersistenceInterceptor` interface.
...
2.  Store the fully-qualified classname of your interceptor implementation class in a file called :
      ...
      `META-INF/services/net.sovrinhealth.fhir.server.spi.interceptor.FHIRPersistenceInterceptor`
...
Additionally, we [pre-package a number of popular implementation guides](https://linuxforhealth.github.io/FHIR/guides/FHIRValidationGuide#optional-profile-support) and make those available from both our GitHub Releases and [Maven Central](https://repo1.maven.org/maven2/net/sovrinhealth/fhir/).
...
### 4.7.4 Sample usage
For examples on how to use the LinuxForHealth FHIR Client, look for tests like `net.sovrinhealth.fhir.client.test.mains.FHIRClientSample` from the `fhir-client` project in git. Additionally, the FHIR Client is heavilly used from our integration tests in `fhir-server-test`.
...
```
"audit": {
    "serviceClassName" : "net.sovrinhealth.fhir.audit.impl.KafkaService",
    "serviceProperties" : {
        "load": "config",
    }
```
...
|`fhirServer/audit/serviceClassName`|string|The audit service to use. Currently, net.sovrinhealth.fhir.audit.impl.NopService to indicate the logger service is disabled, and net.sovrinhealth.fhir.audit.impl.KafkaService to indicate using Kafka as a destination.|
...
|`fhirServer/persistence/factoryClassname`|net.sovrinhealth.fhir.persistence.jdbc.FHIRPersistenceJDBCFactory|
