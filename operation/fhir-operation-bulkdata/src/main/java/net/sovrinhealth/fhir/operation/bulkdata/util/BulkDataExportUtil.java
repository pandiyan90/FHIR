...
import net.sovrinhealth.fhir.operation.bulkdata.model.PollingLocationResponse;
...
public class BulkDataExportUtil {
    ...
    public MediaType checkAndConvertToMediaType(Parameters parameters) throws FHIROperationException {
        ...
        if (parameter.isPresent() && parameter.get().getValue().is(net.sovrinhealth.fhir.model.type.String.class)) {
            mediaType = retrieveOutputFormat(parameter.get().getValue().as(net.sovrinhealth.fhir.model.type.String.class).getValue());
        }
            ...
            for (Parameters.Parameter parameter : parameters.getParameter()) {
                ...
                if (OperationConstants.PARAM_SINCE.equals(parameter.getName().getValue())) {
                    if (parameter.getValue() != null) {
                        if (parameter.getValue().is(net.sovrinhealth.fhir.model.type.String.class)) {
                            return Instant.of(parameter.getValue().as(net.sovrinhealth.fhir.model.type.String.class).getValue());
                        } else if (parameter.getValue().is(net.sovrinhealth.fhir.model.type.Instant.class)) {
                            return Instant.of(parameter.getValue().as(net.sovrinhealth.fhir.model.type.Instant.class).getValue());
                        }
                    }
