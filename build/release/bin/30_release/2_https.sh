...
function deploy_via_curl {
    ...
    do
        ...
        then
            FILE="${POM_FILE}"
            FILE_TARGET_PATH="/net/sovrinhealth/fhir/${MODULE}/${BUILD_VERSION}/${MODULE}-${BUILD_VERSION}.pom"
        ...
        do
            ...
            FILE_TARGET_PATH="/net/sovrinhealth/fhir/${MODULE}/${BUILD_VERSION}/${MODULE}-${BUILD_VERSION}-sources.jar"
            upload_to_https "${MODULE}" "${FILE}" "${FILE_TARGET_PATH}"
        done 
        ...
        do
            ...
            FILE_TARGET_PATH="/net/sovrinhealth/fhir/${MODULE}/${BUILD_VERSION}/${MODULE}-${BUILD_VERSION}-javadoc.jar"
            upload_to_https "${MODULE}" "${FILE}" "${FILE_TARGET_PATH}"
        done
        ...
        do
            FILE="${PROJ}/${TESTS_JAR}"
            FILE_TARGET_PATH="/net/sovrinhealth/fhir/${MODULE}/${BUILD_VERSION}/${MODULE}-${BUILD_VERSION}-tests.jar"
            upload_to_https "${MODULE}" "${FILE}" "${FILE_TARGET_PATH}"
        done
        ...
        do
            FILE="${PROJ}/${JAR}"
            FILE_TARGET_PATH="/net/sovrinhealth/fhir/${MODULE}/${BUILD_VERSION}/${JAR}"
            upload_to_https "${MODULE}" "${FILE}" "${FILE_TARGET_PATH}"
        done
...
function deploy_zip_files { 
    ...
    FILE_TARGET_PATH="/net/sovrinhealth/fhir/${MODULE}/${BUILD_VERSION}/fhir-validation-distribution.zip"
    upload_to_https "${MODULE}" "${FILE}" "${FILE_TARGET_PATH}"
}
